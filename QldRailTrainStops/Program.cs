 ...
 * QLD Railways Senior Software Systems Engineer (4724) role.	*/

namespace QldRailTrainStops
{
    class Program
    {
        /// <summary>
        ...
        static void Main()
        {
            ...
            if (File.Exists(textFile))
            {
                ...
                string output = CalculateSequence(map);
        ...
        private static string CalculateSequence(List<Train> map)
        {
            ...
            if (isValidated)
            {
                ...
                else if (map.Count(x => x.WillStop) > 4) // This is for compound description of Express Stops.
                {
                    ...
                    while (map.Count > 0)
                    {
                        if (map.Count(x => x.WillStop) > 3)
                        {
                            ...
                            var orderedList = map.Where(x => x.OrderIndex <= firstSequenceLimit.OrderIndex).ToList();
                            stringBuilder.Add(CalculateSequence(orderedList));
